...
function signature, API endpoint, CLI flag, config option, return
value -- should behave the way its user would reasonably expect given
its name, context, and the conventions of the surrounding system. When
two interpretations are possible, choose the one that surprises fewer
people.
