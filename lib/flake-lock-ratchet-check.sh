...
(( unique > 0 )) || { echo "FAIL: lock has no identifiable nodes"; exit 1; }
...
fail=0
if (( bytes > max_bytes )); then echo "FAIL: lock is $bytes bytes (sanity cap $max_bytes)"; fail=1; fi
if (( nodes > max_nodes )); then echo "FAIL: lock has $nodes nodes (sanity cap $max_nodes)"; fail=1; fi
if awk -v actual="$ratio" -v cap="$max_ratio" 'BEGIN { exit !(actual > cap) }'; then
  echo "FAIL: lock duplication ratio $ratio exceeds sanity cap $max_ratio"; fail=1
fi
