...
#   BEFORE_REQUIRED_STATUS_CONTEXTS
#                         pipe-separated contexts currently required by the
#                         consumer's default branch.  When supplied, the
#                         migration report includes the context delta.
#   MIGRATION_COVERAGE_REPORT
#                         report path (default: migration-coverage.md).
#
...
trap 'rm -f "$old_checks" "$new_checks" "$old_contexts" "$new_contexts" "${saved_vendored:-}"' EXIT

# Required contexts are a second, independently enforced check surface.  The
# API caller supplies the current list; the standard-derived list is the
# post-migration list.  Keep this optional so offline migrations remain
# usable, while making the omission visible in the report.
printf '%s\n' "${BEFORE_REQUIRED_STATUS_CONTEXTS:-}" | tr '|' '\n' | sed '/^$/d' | sort -u >"$old_contexts"
printf '%s\n' "${REQUIRED_STATUS_CONTEXTS:-}" | tr '|' '\n' | sed '/^$/d' | sort -u >"$new_contexts"
...
if [ -n "$dropped" ]; then
  # Even a refused migration leaves the evidence needed for the PR review.
  # The report is intentionally staged before exiting so a caller may retain
  # it as the explicit loss ledger requested by the migration protocol.
  coverage_report="${MIGRATION_COVERAGE_REPORT:-migration-coverage.md}"
  {
    echo "# Migration check coverage"
    echo
    echo "## Before"
    echo
    echo '```text'; cat "$old_checks"; cat "$old_contexts" 2>/dev/null; echo '```'
    echo
    echo "## After (proposed)"
    echo
    echo '```text'; cat "$new_checks"; cat "$new_contexts" 2>/dev/null; echo '```'
    echo
    echo "## Delta"
    echo
    echo '```text'
...
# Leave a self-contained delta for the migration PR description.  This is
# deliberately plain Markdown so it can be pasted verbatim and reviewed by a
# human; a missing old context is a rename candidate, never silently ignored.
coverage_report="${MIGRATION_COVERAGE_REPORT:-migration-coverage.md}"
{
  echo "# Migration check coverage"
  echo
  echo "Generated by \\`lib/migrate.sh\\`."
  echo
  echo "## Before"
  echo
  echo '```text'
