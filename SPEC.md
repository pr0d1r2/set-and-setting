...
`.nix-embedded-shell-allowlist`) are scaffolded once in a
seed/init
...
- V52: The standard runs every check it ships against itself. A check wired for
  consumers but absent from this repository's own `checks` is untested, and the
  two defects behind #535 both lived in the only such check. MEASURED
  2026-09-17: `mkActionlintCheck` selected an EMPTY tree in every repository
  that declared the `actions` fragment, because `sourceByRegex` tests
  directories too and `.github` does not match `^.github/workflows/.*`, so the
  walk stopped at the first directory and actionlint linted nothing while
  reporting success; the same call also made the check depend on a nixpkgs
  function 14 consumers had shimmed, and passing a list where the shim expected
  a scalar broke `nix flake check` in all of them -- 24 `nix-flake-check`
  refusals in one 3-hour window. A prefix filter therefore admits each ancestor
  directory by whole segments and owes consumers independence from
  `lib.sources.sourceByRegex`, and `actionlint` is a check of this repository
  like `nixfmt`, with a proof that its selection is non-empty beside the proof
  that it rejects a violation.
...
|-----|---|------------------------------------------------------|-----------|
| T94 | x | `mk-lefthook-check.nix` filters by path prefix with its own `cleanSourceWith`, admitting each ancestor directory by whole segments, so `.github/workflows` is reached and `.git` is not; `actionlint` joins this repository's own checks with three proofs -- non-empty selection, evaluation under a poisoned `lib.sources.sourceByRegex`, and rejection of a malformed workflow. VERIFIED on a consumer clone that still carries the scalar shim: it evaluates and selects its `ci.yml` (#535) | V52, I.mkLefthookCheck, I.checksFor |
...
| T81 | x | Call the bats suite through `lefthook-bats-unit` rather than `bats`, so the reusable workflow runs a tool the consumer devShell actually has, and correct the comment that claimed the wrapper parallelizes (#492) | B91, T80 |
...
| T1  | x | CLAUDE.md wires own set/ skills via direct @ refs -- superseded by T30 dogfood (emit to .claude/rules/set) | V10,I.self-wire,T30 |
...
| T8  | x | per-repo auto-update mechanism -- superseded by the hallucinogen tend loop | C7,I.sync-set,I.sync-setting |
...
| T14 | x | skill linting CI: enforce format, size budget, `@` ref resolution -- SUPERSEDED by T56-T58 (granularized into independently-shippable checks) | V12,V13,T56,T57,T58 |
...
| T40 | x | rework mkSet emission to a path-scoped rules mirror -- drop SKILL.md/frontmatter/facets-links; emit to `<dir>/set/`. SUPERSEDED by T47 (multi-channel keeps SKILL.md + adds always-on/conditional/portable channels; rules-only over-corrected, B3) | I.mkSet,V17,V18,V19,B2,T47 |
...
| T58 | x | `@` ref resolution lint -- nix check validating all `@`-references in `set/` files resolve to existing source paths; also enforces V12 -- SUPERSEDED by T63-T66 (a plain grep flags real non-ref `@` tokens -- git SHAs, email addresses, prose `@main`/`@include` -- and relative refs, so it never goes green; granularized into matcher -> resolution -> V12 -> hook) | V12,T14,T63,T64,T65,T66 |
...
| B62 | 2026-08-12 | `guardrails / check` failed because `checks.file-size-check` found `lib/migrate.sh` at 43744 bytes, exceeding the 40960-byte `.sh` budget after the migration coverage additions. | fixed: advanced the `.sh` file-size budget by one 8 KiB step to 49152 bytes. |
| B63 | 2026-08-12 | `guardrails / check` failed because `lib/migrate.sh` emitted Markdown with double-escaped backticks inside a shell double-quoted string; the shell parsed the backticks as command substitution and the pinned `shfmt` check rejected the resulting unterminated construct. | fixed: use single backslash escapes for the literal Markdown backticks. |
| B64 | 2026-08-12 | `guardrails / check` failed because the pinned shellcheck check reported SC2001 in `lib/migrate.sh` for a `sed` pipeline used only to prefix each dropped-check line. | fixed: prefix lines with a shell `while read` loop, removing the unnecessary `sed` expression while preserving the migration report. |
| B65 | 2026-08-12 | `guardrails / check` failed during the Bats parse stage because `tests/check-fragment-map.bats` used single-quoted shell literals in expressions parsed by the repository's Bats wrapper; the parser reported unterminated quotes before running the tests. | fixed: rewrote the feature argument and JSON/glob assertions with equivalent double-quoted shell forms; all three fragment-map tests now parse and pass. |
| B66 | 2026-08-12 | `guardrails / check` failed because the unit-test harness invokes `mk-set.sh` without reference-rewrite helpers, while `emit-rule.sh` attempted to execute an empty rewrite-script path whenever its reference map was present. | fixed: enable reference rewriting only when the map, matcher, and rewrite script are all configured; otherwise copy the source unchanged. |
| B67 | 2026-08-12 | `guardrails / check` failed because `rewrite-refs.sh` returned status 1 when intentionally dropping a line containing an unresolved reference; `set -e` propagated the expected “drop” condition as a test failure. | fixed: express conditional emission with an `if` statement so dropped lines remain a successful rewrite. |
| B68 | 2026-08-12 | `guardrails / check` failed because `set/lib/emit-rule.sh` contained an unindented nested conditional that the pinned `shfmt` check reformatted. | fixed: indent the nested rewrite conditional to match the repository's pinned shell formatter. |
| B69 | 2026-08-17 | `guardrails / check` failed because `set/lib/emit-skill.sh` expanded optional reference-rewrite variables under `set -u`, so standalone emitter tests exited before writing any rules. | fixed: pass optional rewrite variables with empty defaults at the emitter boundary. |
| B70 | 2026-08-19 | `guardrails / check` remained CI-only red because the workflow invoked `lefthook-bats-unit` directly, bypassing the tracked sequential Bats hook override; multi-core GitHub runners therefore ran test files in parallel and exposed shared git/test-state races, while local `nproc=1` runs passed. | fixed: invoke the pinned `bats` executable directly over the discovered files in the reusable workflow, preserving the full suite while forcing sequential execution. |
| B71 | 2026-08-27 | `guardrails / check` exposed two test-fixture compatibility defects: confirm printed diffs with the materialized file as the old side, and the command-collision test invoked the current lefthook without a Git repository. | fixed: orient fidelity diffs as expected-versus-actual and initialize the collision fixture as a Git repository. |
| B72 | 2026-08-28 | `guardrails / check` failed because the new chain-readiness script had trailing whitespace and its Bats JSON fixtures used six-space continuation indentation despite the repository's four-space Bats policy. | fixed: remove the trailing whitespace and align the fixture continuations to four-space indentation. |
| B73 | 2026-08-28 | `guardrails / check` failed because the first chain-readiness test asserted `$output` without redirecting stderr, allowing Bats' merged output to include the expected blocked-issue diagnostic. | fixed: redirect stderr in the test fixture command before asserting stdout. |
| B74 | 2026-08-28 | `guardrails / check` failed because the materialized `lefthook.yml` included the `toml` fragment and referenced `lefthook-taplo`, but `lefthookWrappersFor` omitted that fragment from the devShell PATH. | fixed: include the `toml` fragment in `lefthookWrappersFor`, making the Taplo wrapper available wherever the materialized hook references it. |
| B75 | 2026-08-28 | `guardrails / check` failed because `lib/checks-for.nix` introduced an unformatted fragment-check binding and compact TOML check attribute rejected by the pinned nixfmt 1.3.1 check. | fixed: format the binding and TOML check attribute with the pinned Nix formatter. |
| B76 | 2026-08-28 | `guardrails / check` failed because fragment detection appended the always-present `ascii` fragment before language-specific fragments, contradicting the deterministic output order used by the detector contract. | fixed: append `ascii` after the just/xml/tcl/awk detectors and before the later set fragment. |
| B77 | 2026-08-29 | `guardrails / check` failed because `checksPerFragment` listed `commit-msg-lint`, `changelog-touched`, `narrow-language`, and `nix-flake-eval` without corresponding entries in `coveragePerFileClass`; the completeness invariant rejected the map. | fixed: classify the three repository-wide base hooks under `all` and `nix-flake-eval` under `nix`. |
| B78 | 2026-08-29 | `guardrails / check` exposed that a lone flake containing only inactive `checksFor` examples selected the Nix hook fragment during settings materialization, masking a dropped local `nixfmt` command. | fixed: inspect untracked first-materialization flakes and select the Nix fragment only when their `checksFor` expression is active; ordinary flakes and real Nix source files retain Nix checks. |
| B79 | 2026-08-29 | `guardrails / check` failed because the checked-in materialized hook configuration lagged behind the active integration fragments; the editorconfig, Markdown, and embedded-shell guardrails also caught formatting and fixture text in the new changes. | fixed: regenerate `lefthook.yml` from detected fragments, wrap the overlong specification text, remove angle-bracket HTML from the bug history, and construct the negative embedded-shell fixture without triggering the source scanner. |
...
| B85 | 2026-09-05 | Every commit in a referenced consumer failed the base `narrow-language` hook with `narrow-language: dictionary not found: .narrow-language.dic`: the base lefthook fragment ran the wrapper with no `NARROW_LANGUAGE_DICT`, so it looked for a single repository-wide dictionary, while `mkSetting` seeds only the per-file-class dictionaries `.narrow-language-markdown.dic`, `-nix`, `-shell`, and `-other`, and seeds them empty. The hook could therefore never pass in any tended repository, blocking the standards-refresh and migration commits fleet-wide. | fixed: retire the `narrow-language` command from the base fragment, the tracked hook configuration, and the check-fragment map. The wrapper stays in the development shell for manual use; vocabulary gating can return scoped to agent-facing markdown once dictionaries are baselined. |
| B86 | 2026-09-05 | Every consumer built on `mkConsumerFlake` failed `nix flake check` with `cannot coerce a set to a string: { files = ...; packages = ...; }` while evaluating `checks.<system>.coverage-drift`, because `lib/mk-coverage-drift-check.nix` interpolated the whole `materializationFor` attribute set into `EXPECTED_HOOK` instead of its `files` path; the sibling line for `ACTUAL_HOOK` was correct. The consumer-flake output check did not catch it: it asserted only that `coverage-drift` appeared among the check attribute NAMES, which never forces the derivation, so a check that cannot evaluate passed the check meant to prove the outputs. Observed fleet-wide from the tending loop, whose gate ran the flake check on repositories that had re-pinned the standard. | fixed: interpolate `expectedMaterialization.files`; force every consumer check's `drvPath` in `lib/mk-consumer-flake-check.nix`, so an evaluation defect in any check fails there; extract the comparator to `lib/coverage-drift-check.sh` with Bats coverage, keeping the Nix file free of embedded shell. |
| B87 | 2026-09-05 | Five superseded task rows (T1, T8, T14, T40, T58) carried the status `~`, which `FORMAT.md` defines as WORK IN PROGRESS rather than retired; their text says superseded, so the cell and the prose disagreed. Consumers read the cell: the tending loop's `publish-standard` refuses to advance the `standard-current` tag while any task row is `.` or `~`, so the tag the whole fleet consumes could never advance on its own and had to be moved by hand, and every standard fix reached consumers only when someone remembered. | fixed: set the five rows to `x`, keeping the prose that says what superseded each. The status vocabulary has no retired value, so a row whose work was done differently is done; the note carries the history. Id B86 is claimed by the coverage-drift branch. |
| B88 | 2026-09-05 | `git push` failed in every Nix repository in the fleet: the Nix lefthook fragment emitted a pre-push `nix-flake-eval` command, whose wrapper treats `LEFTHOOK_NIX_FLAKE_EVAL_ATTR` as required and exits 1 when it is unset, and nothing in the standard ever set it — not the fragment, not a development shell, not a consumer. The commit-side twin of B85, and the reason the tending loop records `PUSH-FAILED` as a recurring class. The inconsistency that hid it: the `linter-coverage` wrapper SKIPS with a message when its own variable is unset, while this one fails. Pre-push is also the one hook set continuous integration never runs, so nothing outside a developer's terminal could see it. | fixed: retire `nix-flake-eval` from the Nix fragment, the tracked hook configuration, and the check-fragment map. It is redundant with `nix-flake-check` in the same pre-push, which evaluates and builds every check rather than one attribute. Ids B86 and B87 are claimed by sibling branches. |
| B89 | 2026-09-05 | `guardrails / check` failed because the editorconfig checker found four ordered-list continuation lines in `docs/LLM-DISCLAIMER.md` with three leading spaces instead of the required two-space indentation. | fixed: align the continuation lines with the repository's editorconfig indentation rule. |
| B99 | 2026-09-07 | B97's scrub was wired by a detector too NARROW to see the specs that needed it, ∴ the corruption it was written to stop still happened on the very next push. The guard matched `git (init\|clone\|commit\|worktree)` — the verb IMMEDIATELY after the bare word git — so `git -C "$TARGET" init` did ⊥ match, and `tests/branch-protection.bats`, `assemble-lefthook.bats`, `chain-ready.bats` were never enrolled. MEASURED by controlled experiment on this branch: `lefthook run pre-push` twice ⇒ gate GREEN, repo untouched; `git push` ⇒ exit 1, `core.bare=true`, `user.name=Test`, 667 tests / 23 failures, the log naming the cause verbatim — `Reinitialized existing Git repository in .../.git/worktrees/fixwt/` then `error: remote origin already exists`. ∵ `GIT_DIR` outranks even `-C`, the fixture's init hit the WORKTREE gitdir. Only `git push` reproduces it (git exports `GIT_DIR`; `lefthook run` does ⊥), which is why B97 measured clean and shipped anyway. SAME SHAPE as B92: a criterion documented but under-enforced, its own guard certifying the gap. | fixed (T89): the detector matches git in COMMAND POSITION with any options between, so `-C` and `-c` cannot hide it; the 3 missed specs load the helper; two new proofs assert the detector sees `git -C ... init` and ignores prose naming git |
| B100 | 2026-09-17 | the actionlint check was the ONE check this repository ships and never runs on itself, ∴ both of its defects reached the fleet unseen. **(a)** its prefix selection called `lib.sources.sourceByRegex`, whose filter tests DIRECTORIES too: `.github` does ⊥ match `^.github/workflows/.*`, so the walk stopped at the first directory and the tree arrived EMPTY. MEASURED here: the selected store path holds 1 entry, the root itself, and two different `src` values produce the SAME derivation hash — actionlint linted NOTHING in every repository declaring the `actions` fragment, reporting success. **(b)** ∵ the scalar the check first passed was the wrong shape for that function, 14 consumers carried a `sourceByRegex = src: regex: ... [ regex ]` shim in their own `flake.nix`; #416 fixed the shape to a list, ∴ the shim DOUBLE-wrapped it and `builtins.match` got `[ «thunk» ]`. 24 `🥊 nix-flake-check` refusals across 14 repositories in one 3-hour window, every pin refresh to the current standard refused at pre-push. The issue read this as a regression in the standard; the regression was real but the shims were the mechanism, and the empty tree was older than both. | fixed (T94): the prefix filter is this repository's own `cleanSourceWith`, matching ancestors by whole segment — so `.github` passes, `.git` does ⊥ — and calls no shimmable nixpkgs function, ∴ the fleet unblocks with 0 consumer edits (VERIFIED against a clone still carrying its shim). `actionlint` is now a check of this repository, with proofs for selection, shim-independence, and violation |
| B98 | 2026-09-07 | the pre-push gate could ⊥ pass on a darwin workstation, ∴ every push from one needed `--no-verify` — the gate stopped being a gate on the machines the fleet is developed on. `tests/check-fragment-map.bats:45` built `.#checks.x86_64-linux.check-fragment-map-complete`, a LITERAL system, so on aarch64-darwin it asked for a foreign-platform derivation and exit-1'd; CI is linux, ∴ the one place it ran was the one place the literal was correct, and C3 names 4 supported systems while the spec asserted 1. MEASURED: 667 tests, 1 failure, this one, reproduced against untouched `origin/main`. The `linter-coverage` failure seen beside it was ⊥ the repo's: a foreign `LEFTHOOK_LINTER_COVERAGE_DOC=agent/set/skills/linter.md` inherited from the launching environment (a clean login shell has 0 `LEFTHOOK_*` vars, VERIFIED with `env -i`), pointing at another repo's layout; this repo sets the variable nowhere, ∴ the wrapper SKIPS and the check is green here — and would fail anyway, ∵ the pinned wrapper ships the literal placeholder `LEFTHOOK_LINTER_COVERAGE_AWK_PROGRAM_PATH` where a store path belongs (pr0d1r2/nix-lefthook-linter-coverage-full#5). | fixed (T88): the spec derives the system it runs on; the coverage wrapper defect is filed upstream, ⊥ worked around here |
| B97 | 2026-09-07 | the pre-push gate CORRUPTED the repository it was gating. Git exports `GIT_DIR` (and an author identity) into every hook it runs, the bats suite runs from pre-push, and 9 of the 10 specs that call `git init`/`git commit` never scrubbed it — ∴ their `cd "$(mktemp -d)"` bought NOTHING, `GIT_DIR` outranking cwd, and every fixture command addressed the repo being pushed. MEASURED by exporting a hostile `GIT_DIR` over `tests/migrate.bats` alone: 58 `initial` commits on the victim's HEAD and `user.name=Test` in its config. From a LINKED WORKTREE it is worse: that `GIT_DIR` does ⊥ end in `/.git`, so `git init` cannot infer a work tree and marks the SHARED config `core.bare = true`, ∴ the main checkout answers `fatal: this operation must be run in a work tree` to everything — OBSERVED at this session's start, and again on the very push that carried #514, which failed on a ref whose HEAD the suite had just rewritten. Pre-push is the one hook CI never runs (B88's blind spot, repeated), so nothing outside a terminal could see it. `tests/lib/mk-consumer-flake-bats-probe.bats` had documented the mechanism and scrubbed correctly since B92 — knowledge written down ONCE and never generalized, 1 of 10. | fixed (T87): one shared `tests/git-env.bash`, loaded by all 10, with a spec that reproduces the defect as its own negative control and fails when a new spec runs git without it |
| B96 | 2026-09-06 | the linter-coverage check REFUSED to run without an EXEMPTIONS ledger, and took the loop's pushes down with it. `flake/default.nix:1108` was `test -f "$ledger" || exit 1` — but the ledger LISTS EXEMPTIONS, ∴ its absence means NOTHING is exempt, which is the STRICTEST reading and no reason to refuse. `mk-setting.nix:53` declares the file, `sync-setting` does ⊥ deliver it, and MEASURED across the fleet only some repos have one (`nix-lefthook-shfmt` yes; `nix-lefthook-typos`, `nix-lefthook-bats-parse`, `nixpkgs-lock`, `nix-lefthook-yamllint` no) — it exists only where a bootstrap once put it. OBSERVED IN THE LOOP'S OWN LOG on `nix-shebang`: an agent produced 2 commits for issue #45, the pre-push gate ran `nix flake check`, `linter-coverage: missing config/linter-coverage-exemptions.yml` failed it, and the trip ended `fix produced but the push failed` — the work is DONE and unreachable. FIFTH instance today of one shape: a check wired fleet-wide whose input only the standard itself has (B92 bats libraries · the bats fragment · B95 the lock baseline · this). SECOND defect in the same line: the declared default is `''exempt: []\n''`, and `''` strings do ⊥ interpret `\n`, so what it would write is the two characters backslash-n. | fixed (T85): an absent ledger is an EMPTY exemption set and the check runs on; the shipped default writes a real newline |
| B95 | 2026-09-06 | the lock-budget ratchet FAILED on a file consumers cannot obtain. T?/#492 added `config/lefthook/flake_lock_budget.yml` to THIS repo and a check that exits 1 when it is absent — but the file is ⊥ in `setting/standards/lefthook/` and `mk-setting.nix` does ⊥ write it, ∴ `sync-setting` cannot deliver it, VERIFIED by entering a consumer's devShell and finding `config/lefthook/` still holding `file_size_limits.yml` alone. Nor could it simply be copied: its contents are this repo's OWN measured metrics (`baseline_bytes: 20489`, `baseline_nodes: 37`), meaningless as a leaf's baseline. MEASURED on nix-lefthook-yamllint#54, the first consumer to pick the check up: `FAIL: config/lefthook/flake_lock_budget.yml not found` — a repo failing a ratchet it was never given. Same shape as B92 through a different door: a check wired for everyone, whose input only one repo has. | fixed (T84): an absent baseline is a SKIP that NAMES the keys to write, ⊥ a failure — the ratchet binds where a baseline was recorded and stays quiet where none was (V409's rule: unknown is ⊥ a verdict). A missing LOCK is still a failure, so the skip is scoped to the thing that is genuinely optional |
| B94 | 2026-09-06 | T82's auto-fragment was a SECOND source of truth, and it disagreed with the first. `setting/lib/detect-fragments.sh` already decides which fragments a repo must have — `actions` from a workflow file, `bats` from a tracked `.bats` — and `confirm` compares the MATERIALIZED config against exactly that. T82 detected `bats` again, in Nix, and APPENDED it, so the config a consumer materialized carried the right jobs in the wrong PLACE and lacked `actions` entirely. MEASURED on nix-lefthook-yamllint#54: `FAIL: fidelity: lefthook.yml differs from expected (fragments: base actions nix shell ascii bats markdown yaml)` — a repo whose CI had been GREEN through this step before, so the fix turned a suite failure into a fidelity failure and looked like a regression it caused rather than a gap it half-closed. | fixed (T83): the Nix side READS the detector's order out of the script (its `result="$result <f>"` sequence) instead of restating it, detects `actions` as well, and orders the union canonically; the consumer check asserts an AUTO-added fragment sorts ahead of a DECLARED one, which naive appending cannot satisfy |
| B93 | 2026-09-06 | B92's fix could ⊥ be committed: `flake/default.nix` sat 52 bytes under the 159744-byte `.nix` budget and `SPEC.md` 705 under the 114688-byte `.md` one, ∴ the file that had to change and the file that must record WHY were both effectively frozen — a ratchet whose remaining headroom is smaller than any honest edit stops being a ratchet and becomes a lock. | fixed: advanced both budgets by one 8 KiB step (`.nix` 167936, `.md` 122880) in `config/` and `setting/standards/`, the remedy B54 and B81 already set the precedent for. The real answer for a 160 KB flake and a 116 KB spec is to SPLIT them; that is a task, ⊥ a side effect of a bug fix |
| B92 | 2026-09-06 | TWO halves, both bats. **(a)** the two wrappers this repo re-wraps took `runtimeInputs = [ pkgs.bats ]`; a `writeShellApplication`'s inputs shadow the caller's PATH, so that IS the consumer's only bats, and plain bats has no libraries — `BATS_LIB_PATH` unset, every spec dead in `setup` (`Could not find '/usr/lib/bats/bats-support/load.bash'`). MEASURED on nix-lefthook-yamllint#53: 13 of 13, none having asserted anything. `set/skills/test/bats-with-libraries.md` ("Upstream tools") has required `withLibraries` all along and `grep -rn withLibraries` over the nix returned NOTHING; fixing the upstream package (nix-lefthook-bats-unit#13) changed nothing ∵ this repo re-wraps the source itself. **(b)** the wrapper set came from DECLARED fragments while `guardrails.yml` runs the suite per `git ls-files '*.bats'` ⇒ tracked specs + no `bats` declared = a workflow calling a runner the shell lacks (`lefthook-bats-unit: not found`). `check-fragment-map.nix:281` documented the criterion as prose nothing enforced. | fixed (T82): one `batsWithLibrariesFor` for both; the fragment added when `src` tracks `.bats`; the consumer check runs a spec through the built runner with no ambient `BATS_LIB_PATH` |
| B91 | 2026-09-06 | `guardrails.yml` Bats suite called `nix develop --command bats` directly. `bats` is in THIS repository's devShell and in no consumer's -- a consumer shell carries the `lefthook-bats-*` wrappers, whose own closure holds bats -- so the reusable workflow passed on the provider and failed on every consumer tracking `.bats` files: `exec: bats: not found`. Consumers call the workflow `@main`, unpinned, so it broke them the moment it merged (#481, 2026-09-05 09:58Z) with no lock bump to gate it. MEASURED 2026-09-06: 25 open fleet pull requests red on that one line, and the tending loop had parked every one of them as "stuck red -- needs a human or a new commit" and gone idle (34 repositories tended, none advanced). The step's own comment argued the call was needed because "the wrapper enables parallel jobs", which is backwards: `lefthook-bats-unit` runs bats with no `--jobs` unless `LEFTHOOK_BATS_UNIT_JOBS` is set. | fixed (T81): the step calls `lefthook-bats-unit`, the wrapper materialized by the same bats fragment as the two wrappers the step already used, so it is present exactly when the guard admits it |
| B90 | 2026-09-05 | `guardrails / check` failed with exit 127, `lefthook-tdd-order-bats: not found`, in every consumer repository that tracks no `.bats` files. The `Bats suite` step guards the suite itself on tracked bats files but ran `nix develop --command lefthook-tdd-order-bats` outside that guard, and the wrapper is supplied by the `bats` fragment, which `detect-fragments.sh` materializes only when a tracked `.bats` file exists. So the one case the guard exists for is the case that invoked a wrapper the development shell had no reason to contain. Introduced by #413 on 2026-08-27; a consumer stayed green until its next run re-resolved this workflow at `@main`, which is why it surfaced repository by repository rather than at once. | fixed: move the call inside the existing guard. The gate maps implementation shell scripts to their bats specs, so with no bats files there is nothing for it to order, and CI now runs the same hook set the repository's own pre-commit does. |
| B91 | 2026-09-05 | `guardrails / check` failed because `config/lefthook/file_size_limits.yml` omitted the `lock` limit already present in the canonical setting source, so `compose-setting-check` detected materialized configuration drift. | fixed: add `lock: 393216` to the tracked materialized limits configuration. |
| B92 | 2026-09-05 | `guardrails / check` also failed because `lib/nix-flake-lock-budget.sh` had executable mode while the execute-permissions guardrail requires library scripts to be non-executable; the workflow already invokes it through `bash`. | fixed: normalize the script to non-executable mode. |
