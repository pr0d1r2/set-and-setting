...
|-----|---|------------------------------------------------------|-----------|
| T82 | x | RETARGETED -- fix the wrapper, ⊥ move the tree. Moving all 617 specs was rejected as too much for the defect, which was never the layout: `nix-lefthook-bats-unit#12` discovers with `git ls-files`, runs sequentially, and refuses outside a repository. `tests/unit/` stays a convention new specs may follow; no repository owes a migration. | V1,B91,B16,B18,B70 |
| T83 | x | HOOTL-ELIGIBLE -- CI wall-clock is a lever on tending velocity, and two changes take it without touching what the gate checks (#489). (a) `concurrency` with `cancel-in-progress` on the reusable workflow: the loop force-pushes every fix round, so a superseded run kept a runner busy and competed with the run that mattered. Scoped `github.event_name == 'pull_request'` -- cancelling on the default branch would kill the `cache-push` that follows a merge -- and grouped per `github.ref`, ∵ a repository-wide group would serialise unrelated pull requests. (b) `--keep-going` on the flake check: all-or-nothing means one failure per round, and a ROUND TRIP is the loop's unit of cost, ⊥ a second. MEASURED with two planted violations: without it the run surfaced ONE problem and stopped; with it, FOUR attributes were named in a single run. Deliberately excluded: scoping darwin off pull requests (a coverage trade for a human) and sharding the check across runners (nix is already parallel within a job via `max-jobs = auto`). + bats (cancellation exists, is per-ref, is limited to pull requests and is ⊥ a bare `true`; both platform jobs still run unchained; `--keep-going` and `--print-build-logs` reach the runner) | I.flake,V1,T81,B70 |
| T81 | x | HOOTL-ELIGIBLE -- lint the shell that CI actually runs, and stop hiding it in YAML. `actionlint` shellchecks every `run:` block, but ONLY when `shellcheck` is on its PATH; the pinned wrapper carried `actionlint` alone, so that integration was inert and workflow shell was linted by nothing — while `*.sh` in this repository passes shellcheck, shfmt, no-shell-functions, editorconfig and a Bats suite. Add `pkgs.shellcheck` to the wrapper, and move the six `run:` blocks into tracked `.github/scripts/*.sh`: the flake check (one script, previously four identical lines in each platform job), the sequential Bats suite (B16/B18/B70), the darwin materialization, and the cache-push build. The workflows keep the GitHub expressions they need — the flake-check timeout reaches the script as an env var, ⊥ interpolated into shell. Bats coverage asserts the commands the gate depends on, including that no `--jobs` reaches the runner. | I.flake,V1,B12,B16,B18,B70 |
...
| B90 | 2026-09-05 | Giving actionlint its shellcheck grew `flake/default.nix` to 159804 bytes, over the 159744-byte `.nix` budget: a comment and a two-item list sufficed, the file sitting 204 bytes under its cap. Class of B13/B21/B31/B54. | fixed: advance the `.nix` budget one 8 KiB step to 167936. With B92: the file is at its size cap AND exempt from the embedded-shell rule, which argues for splitting it, ⊥ for the next step. |
| B91 | 2026-09-05 | The pre-push Bats gate had never run a single test here: `lefthook-bats-unit` defaulted to `tests/unit/` and exited 0 SILENTLY when absent, while this repository keeps its specs flat in `tests/` — green in 0.02s having run nothing. Measured when CI failed a change whose pre-push had just passed. Two defects sat behind it: a non-recursive glob, and `bats --jobs $(nproc)`, flaky here per B16/B18/B70. | fixed upstream in `nix-lefthook-bats-unit#12` (`git ls-files` discovery, sequential, refuses outside a repository) ⊥ by moving 617 files; the pin bump makes the gate run 637 specs. CI was never affected. |
| B92 | 2026-09-05 | `flake/default.nix` fails the `nix-no-embedded-shell` git hook on `main`'s own content (verified against a clean checkout), so the repository's largest Nix file cannot be staged, while `guardrails / check` stays green. The allowlist carries `flake.nix` and `setting/lib/mk-setting.nix`, ⊥ `flake/default.nix`: the monolith split left the entry behind. Hook and pinned check disagree on the same bytes. | fixed: allowlist `flake/default.nix`, aligning the hook with what the pinned check tolerates. The exempted blocks are real and should be extracted, and the two runners reconciled; both are follow-ups. |
| B93 | 2026-09-05 | The T83 entry carried `SPEC.md` to 115925 bytes, over the 114688-byte markdown budget. Class of B6, B14, B20, B30, B58, B81 -- the file grows with every bug and task row by design. | fixed: advance the markdown budget one 8 KiB step to 122880 in both copies, the consumer's and the standard's, since `compose-setting` compares them. The rows added earlier in this branch were compressed rather than bumped; a second squeeze in one branch would cost more in readability than the step costs in slack. |
