...
| B90 | 2026-09-05 | `guardrails / check` failed because the lock-budget shell script used compact conditionals rejected by pinned `shfmt`, its two related files had unexpected execute permissions, and `flake/default.nix` retained an unused lock-budget source argument rejected by `deadnix`. | fixed: format the conditionals with pinned `shfmt`, remove execute permissions from the script and its Bats test, and remove the unused flake argument. |
| B91 | 2026-09-05 | `guardrails / check` failed because `.github/workflows/ci.yml` included a lock-growth marker emoji, which violates the repository-wide ASCII-only guard. | fixed: use an ASCII-only lock-growth marker message. |
