...
| B42 | 2026-07-29 | `guardrails / check-darwin` CI hung indefinitely at the `DeterminateSystems/nix-installer-action@main` step. The `@main` ref is a floating reference that resolves to whatever commit is on the action's default branch at the time of the run. Between two consecutive CI runs on the same PR, `@main` resolved to different commits -- the first run succeeded, the second hung. Same class as using unpinned action versions: non-deterministic CI that can break without any repo change. | fixed: pinned `DeterminateSystems/nix-installer-action` from `@main` to `@v22` (latest stable release) in `guardrails.yml`. Stable version tags are immutable and deterministic, matching the repo's pinned-deps philosophy (C6/C7). |
