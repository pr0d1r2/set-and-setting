...
| B89 | 2026-09-05 | `guardrails / check` failed because `lib/flake-lock-ratchet-check.sh` and its Bats test were tracked with executable permissions, while the execute-permissions guard requires repository source and test files to be non-executable. | fixed: remove the executable bit from both files. |
| B90 | 2026-09-05 | `guardrails / check` failed because `lib/flake-lock-ratchet-check.sh` defined `read_value()`, violating the repository's no-shell-functions guardrail. | fixed: replace the helper with explicit `sed` lookups for each lock-budget field, preserving the ratchet checks without a shell function. |
| B91 | 2026-09-05 | `flake-lock-ratchet-check` test 380 expected a growth notice, but its fixture was only 102 bytes and did not cross the configured 5% threshold. | fixed: make the ordinary-growth fixture include enough metadata to exceed the threshold while preserving its non-duplicated lock shape. |
