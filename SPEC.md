...
|-----|---|------------------------------------------------------|-----------|
| T82 | . | HOOTL-ELIGIBLE -- move this repository's specs to `tests/unit/`, the fleet's convention for unit specs (hallucinogen already sets `LEFTHOOK_TDD_SPEC_DIR=tests/unit`). All 617 specs sit flat in `tests/`, and the two rules currently disagree: `tdd-order-bats` demands `tests/<source-path>.bats` while `lefthook-bats-unit` defaults to `tests/unit/` and globs non-recursively, which is why the pre-push Bats gate has never run anything here (B91). The move is therefore ALL-OR-NOTHING: relocating a few specs makes `tdd-order` reject them, and setting the spec dir before the move makes it reject every existing spec. Migrate the tree, fix the `$BATS_TEST_DIRNAME/..` depths, set the spec dir, and pair it with the wrapper fix B91 names (discover with `git ls-files`, run sequentially, refuse rather than pass on an empty set) so the gate both finds the specs and cannot silently find none. | V1,B91,B16,B18,B70 |
...
| B89 | 2026-09-05 | `flake/default.nix` fails the `nix-no-embedded-shell` git hook on `main`'s own content — verified by running the pinned wrapper against a clean checkout — so the repository's largest Nix file cannot be staged, while `guardrails / check` stays green. The allowlist carries `flake.nix` and `setting/lib/mk-setting.nix` but not `flake/default.nix`: when the monolith was split into a module the entry did not follow it, and nobody noticed because nobody had staged that file since. The hook and the pinned check disagree about the same rule on the same bytes, which is the divergence class B162 and B201 record from the other direction. | fixed: add `flake/default.nix` to `.nix-embedded-shell-allowlist`, aligning the hook with what the pinned check already tolerates. The embedded blocks it exempts (the wrapper placeholder guard and the path matcher) are real and should be extracted, and the two runners should be reconciled so one cannot accept what the other rejects; both are follow-ups, not this change. |
| B90 | 2026-09-05 | `guardrails / check` failed because giving actionlint its shellcheck grew `flake/default.nix` to 159804 bytes, over the 159744-byte `.nix` budget — a one-line comment and a two-item list were enough, because the file sits 204 bytes under its cap. Same class as B13, B21, B31 and B54. | fixed: advance the `.nix` budget by one 8 KiB step to 167936. Worth reading with B89: the file is simultaneously at its size cap and exempted from the embedded-shell rule, which is the argument for splitting it rather than for the next step. |
| B91 | 2026-09-05 | The pre-push Bats gate has never run a single test in this repository. `lefthook-bats-unit` with no arguments defaults to `tests/unit/` and exits 0 SILENTLY when that directory is absent; this repository keeps its specs flat in `tests/`, so the hook reported green in 0.02 seconds having executed nothing. Measured when CI failed `cache-delivery.bats` on a change whose pre-push had just passed. Two further defects sit behind the same default: the glob is NOT recursive, so `tests/<dir>/*.bats` would be missed even with the right directory, and it runs `bats --jobs $(nproc)`, which B16, B18 and B70 all record as flaky here because the files share git and test state. | recorded, not fixed here: the honest fix is in the `nix-lefthook-bats-unit` wrapper (discover with `git ls-files`, run sequentially, and refuse rather than pass when it finds nothing), and this change is about workflow shell. CI runs the full suite via `.github/scripts/bats-suite.sh`, which is sequential and uses `git ls-files`, so the gate that matters is intact. The specs' desired home is `tests/unit/` (the fleet's convention); T82 carries that migration, which cannot be done piecemeal. |
