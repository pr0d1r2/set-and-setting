...
| B40 | 2026-07-22 | `guardrails / check` failed after the referenced-consumer migration because the declared fragment list omitted `set` even though this repository contains set sources, the standalone confirm app could not resolve the markdown and YAML lefthook wrappers supplied by the development shell, and the migrated output lambda retained 29 unused legacy wrapper bindings rejected by deadnix. | fixed: declared the `set` fragment, ran confirmation through the materialized development shell so coherence checks the real wrapper environment, and removed the stale lambda bindings. |
